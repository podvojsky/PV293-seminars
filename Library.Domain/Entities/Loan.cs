...
public class Loan
{
    ...
    public void ExtendDueDate(int additionalDays)
    {
        ...
        if (additionalDays <= 0)
            throw new ArgumentException("Cannot extend loan by negative days");
        if (IsOverdue())
    ...
    {
        if (Status is LoanStatus.Lost or LoanStatus.Completed)
            throw new InvalidOperationException("Cannot report damage on lost or completed loan");

        if (string.IsNullOrEmpty(damageDescription))
            throw new ArgumentException("Damage description cannot be empty");

        if (damageCost.Amount <= 0)
            throw new ArgumentException("Damage amount needs to be a positive number");
